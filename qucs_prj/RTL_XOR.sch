<Qucs Schematic 26.1.1>
<Properties>
  <View=-15,-91,608,592,1.80156,0,0>
  <Grid=10,10,1>
  <DataSet=RTL_XOR.dat>
  <DataDisplay=RTL_XOR.dpl>
  <OpenDisplay=0>
  <Script=RTL_XOR.m>
  <RunScript=0>
  <showFrame=0>
  <FrameText0=Title>
  <FrameText1=Drawn By:>
  <FrameText2=Date:>
  <FrameText3=Revision:>
</Properties>
<Symbol>
  <.ID -50 44 XOR>
  <.PortSym -30 -20 1 0 A>
  <.PortSym -30 20 2 0 B>
  <.PortSym 40 0 3 180 OUT>
  <.PortSym 10 -40 4 270 3v6>
  <.PortSym 10 40 5 90 GND>
  <Line -30 -20 20 0 #000080 2 1>
  <Line -30 20 20 0 #000080 2 1>
  <Line 20 0 20 0 #000080 2 1>
  <EArc -60 -40 80 80 1440 -2880 #00007f 2 1>
  <EArc -30 -40 20 80 1440 -2921 #00007f 2 1>
  <EArc -40 -40 20 80 1440 -2921 #00007f 2 1>
  <Line 10 -40 0 10 #00007f 2 1>
  <Line 10 40 0 -10 #00007f 2 1>
  <Text 10 -15 12 #000000 270 "RTL">
</Symbol>
<Components>
  <Port A 1 50 90 -23 12 0 0 "1" 1 "analog" 0>
  <Port B 1 50 230 -23 12 0 0 "2" 1 "analog" 0>
  <Port OUT 1 400 160 4 -50 0 2 "3" 1 "analog" 0>
  <Port 3v6 1 190 30 -73 -23 0 3 "4" 1 "analog" 0>
  <Port GND 1 300 280 12 4 0 1 "5" 1 "analog" 0>
  <Sub AND1 1 230 110 -50 44 0 0 "RTL_AND.sch" 0>
  <Sub NOR7 1 140 210 -50 44 0 0 "RTL_NOR.sch" 0>
  <Sub NOR8 1 350 160 -50 44 0 0 "RTL_NOR.sch" 0>
</Components>
<Wires>
  <150 250 150 280 "" 0 0 0 "">
  <150 50 150 170 "" 0 0 0 "">
  <290 110 290 140 "" 0 0 0 "">
  <90 130 190 130 "" 0 0 0 "">
  <110 90 190 90 "" 0 0 0 "">
  <110 90 110 190 "" 0 0 0 "">
  <150 50 190 50 "" 0 0 0 "">
  <290 140 320 140 "" 0 0 0 "">
  <360 200 360 280 "" 0 0 0 "">
  <290 180 320 180 "" 0 0 0 "">
  <360 50 360 120 "" 0 0 0 "">
  <90 230 110 230 "" 0 0 0 "">
  <150 280 240 280 "" 0 0 0 "">
  <240 150 240 280 "" 0 0 0 "">
  <290 180 290 210 "" 0 0 0 "">
  <90 130 90 230 "" 0 0 0 "">
  <240 50 240 70 "" 0 0 0 "">
  <270 110 290 110 "" 0 0 0 "">
  <190 210 290 210 "" 0 0 0 "">
  <240 280 300 280 "" 0 0 0 "">
  <240 50 360 50 "" 0 0 0 "">
  <290 90 290 110 "" 0 0 0 "">
  <290 90 400 90 "" 0 0 0 "">
  <190 30 190 50 "" 0 0 0 "">
  <300 280 360 280 "" 0 0 0 "">
  <190 50 240 50 "" 0 0 0 "">
  <50 90 110 90 "" 0 0 0 "">
  <50 230 90 230 "" 0 0 0 "">
  <400 160 400 160 "sum" 410 180 0 "">
</Wires>
<Diagrams>
</Diagrams>
<Paintings>
  <Text 320 0 12 #000000 0 "q = (a&!b) | (!a&b)">
</Paintings>
